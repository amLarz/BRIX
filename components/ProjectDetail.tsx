...
const ProjectDetail: React.FC<ProjectDetailProps> = ({
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <button onClick={onBack} className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-gray-500 hover:text-[#8B3A2B] transition-colors ml-2">
        ← Back to projects
      </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
        ...
        <div className="space-y-4">
          ...
          {project.status !== 'Finished Project' && project.updates && project.updates.length > 0 && (
            <ProjectProgressBar 
              projectName={project.name}
              percentage={project.percentage}
              startDate={project.startDate}
              estimatedEnd={project.deadline}
              updates={project.updates}
            />
          )}

          {/* Materials Table - Moved into left column to fill space */}
          ...
          {project.bidders && project.bidders.length > 0 && (
            <section className="bg-white rounded-3xl overflow-hidden shadow-sm border border-gray-100 mt-4">
              <div className="bg-gray-900 p-6 flex justify-between items-center">
                <h3 className="text-white font-black italic uppercase tracking-widest">Participating Bidders</h3>
                <div className="w-12 h-6 construction-stripes opacity-30" />
              </div>
              <div className="p-0 overflow-x-auto">
                <table className="w-full text-left border-collapse min-w-[400px]">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-100">
                      <th className="px-6 py-4 text-[10px] font-black text-gray-400 uppercase">Bidder / Contractor</th>
                      <th className="px-6 py-4 text-[10px] font-black text-gray-400 uppercase">Bid Amount</th>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {project.bidders.map((bidder, i) => (
                      <tr key={i} className="hover:bg-gray-50/50 transition-colors">
                        <td className="px-6 py-5 font-bold text-gray-900 text-sm">{bidder.name}</td>
                        <td className="px-6 py-5 text-sm font-black text-gray-600">{bidder.amount}</td>
                        <td className="px-6 py-5 text-right">
                          <span className={`text-[10px] font-black uppercase px-3 py-1 rounded-full border ${bidder.status === 'Awarded' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-gray-50 text-gray-500 border-gray-200'}`}>
                            {bidder.status}
                          </span>
                        </td>
                      </tr>
        ...
        {/* Right Column: Information & Discussion */}
        <div className="space-y-4">
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
            ...
            {project.managingOrganization && (
              <div className="mt-4 p-4 bg-[#FDF7E7]/50 rounded-2xl border border-[#8B3A2B]/10">
                <span className="block text-[10px] font-black text-[#8B3A2B] uppercase mb-1 tracking-widest">Managing Organization</span>
                <span className="font-bold text-gray-900 text-sm">{project.managingOrganization}</span>
              </div>
            )}
